...
class NumericalAbstraction:
    ...
    def abstract_numerical(self, data_table, cols, window_size, aggregation_function):
        ...
        # Ensure window_size is a non-negative integer (pandas requires this).
        try:
            win = int(window_size)
        except Exception:
            raise ValueError(f"window_size must be convertible to an integer, got {window_size!r}")
        if win < 0:
            raise ValueError(f"window_size must be an integer 0 or greater, got {win}")

        # Create new columns for the temporal data, pass over the dataset and compute values
        for col in cols:
            data_table[
                col + "_temp_" + aggregation_function + "_ws_" + str(win)
            ] = (
                data_table[col]
                .rolling(win)
